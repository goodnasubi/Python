from janome.tokenizer import Tokenizer # janome.tokenizer
import re
...
def analyze(text):
    ...
    形態素解析を行う
    :param text:  解析対象の文章
    :return: 見出しと品詞のペアを格納した多重リスト
    """
    t = Tokenizer()             #Tokenizerオブジェクトを生成
    tokens = t.tokenize(text)   #形態素解析を行う
    result = []                 #解析結果の見出しと品詞を格納するリスト

    # リストからTokenオブジェクトを１つずつ取り出す
    for token in tokens:
        result.append(
            [
...
def keyword_check(part):
    ...
    :return: 名詞であればTrue、そうでなければFalse
    """
